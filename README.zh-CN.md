...
- 图片、文件、目录在列表右键菜单和详情页中支持“另存为...”。
- 详情页操作区重新优化，支持收藏、分享、来源应用展示、复制路径、在 Finder 中显示和打开等操作。
- 支持图片、文件、文件夹拖拽：可从菜单栏或主面板直接拖到访达、邮件等应用。
- 拖拽预览采用系统访达风格：多文件图标层叠、数量角标，拖拽时源条目变暗。
- 拖出的图片使用原始全分辨率数据，而不是列表缩略图。
- 拖拽的文件或目录已被删除时给出明确提示。
- 优化沙盒文件访问能力，提升另存为、拖拽和文件/目录操作稳定性。
- 应用重构为独立的协调器与图片、文本、文件详情窗口，便于维护。
...
- URL 识别，支持浏览器打开。
- 文件和目录识别，支持详情预览、另存为、复制路径、在 Finder 中显示和打开。
- 支持将图片、文件、目录从列表直接拖拽到访达和其他应用。
- 支持通过 macOS 系统分享面板分享文本、链接、图片、文件和目录。
- 图片预览支持缩放和拖动查看。
- 详情页展示来源应用，并支持复制、分享、收藏和对应类型的快捷操作。
